// --- File: src/app/sitter/[id]/SitterProfileClient.tsx ---
"use client";
...
const formatDateSince = (dateString: string) => {
    try {
        return moment(dateString).locale('ru').format('D MMMM YYYY');
    } catch (e) {
        return dateString;
    }
};
...
const formatLastSeen = (dateString: string | null, isOnline: boolean) => {
    ...
    try {
        return `Был(а) в сети: ${moment(dateString).locale('ru').fromNow()}`;
    } catch (e) {
        return '';
    }
};
...
export default function SitterProfileClient({ initialSitter, sitterId }: { initialSitter: SitterProfileResponse, sitterId: string }) {
    const { t } = useTranslation();
    const router = useRouter();
    const dispatch = useDispatch<AppDispatch>();
    const { isAuthenticated } = useSelector((state: RootState) => state.auth);

    // Данные ситтера берем из пропсов (уже отрендерено сервером!)
    const [sitter] = useState<SitterProfileResponse>(initialSitter);
    ...
    // Модалка авторизации (заменяет useOutletContext из React Router)
    const [authModalOpen, setAuthModalOpen] = useState(false);
    const [authMode, setAuthMode] = useState<'login' | 'register'>('register');

    // Добавляем состояние для отложенного действия
    ...
    // Слушаем авторизацию: если она прошла и мы ждали открытие модалки - открываем
    useEffect(() => {
        if (isAuthenticated && pendingAction === 'OFFER_ORDER') {
            setPendingAction(null);
            handleOfferOrderClick();
        }
    }, [isAuthenticated, pendingAction]);

    // Подгрузка отзывов происходит на клиенте
    useEffect(() => {
        if (sitter.reviews_count > 0) {
            getWorkerReviews(sitterId, 1, 5)
                .then(res => setReviews(res?.data || []))
                .catch(e => console.error("Failed to load reviews", e));
        }
    }, [sitter.reviews_count, sitterId]);
    ...
    const handleOfferOrderClick = async () => {
        if (!isAuthenticated) {
            setPendingAction('OFFER_ORDER');
            // Указываем AuthModal не перекидывать нас в кабинет, а оставить тут
            dispatch(setAuthRedirectPath('NO_REDIRECT'));
    ...
    const openInfo = (title: string, text: string, iconUrl?: string) => {
        setInfoModalData({ title, text, iconUrl });
    };
    ...
    const renderHighlights = () => (
        <div className={style.highlightsGrid}>
            ...
            {sitter.children_under_twelve_yo === 0 ? (
                ...
                <div
                    className={style.highlightItem}
                    onClick={() => openInfo("Дети в доме", "В доме исполнителя проживают детей. Учитывайте это, если ваш питомец не ладит с детьми.")}
                >
                    <div className={style.iconBox}><IconChild /></div>
            ...
            {sitter.certificates && sitter.certificates.length > 0 && sitter.certificates.map((cert: any, index: number) => (
                <div
                    key={cert.slug || index}
                    className={style.highlightItem}
                    onClick={() => openInfo(cert.name, cert.description || "Подтвержденный сертификат.", cert.icon_url)}
                >
                    <div className={style.iconBox}>
                        <img src={cert.icon_url} alt={cert.name} style={{ width: 22, height: 22, objectFit: 'contain' }} />
                    </div>
                    <span>{cert.name}</span>
        ...
        <>
            ...
            <div className={style.pageWrapper}>
                        ...
                        <aside className={style.leftSidebar}>
                            <div className={style.profileCard}>
                                <div className={style.avatarWrapper}>
                                    ...
                                    {sitter.is_premium && (
                                        <div className={style.premiumBadge} title="ПРО Исполнитель (Подтвержден)">
                                            <IconCrown />
                                        </div>
                                    )}
                                </div>
                                <div className={style.profileMeta}>
                                    <div className={style.nameRow}>
                                        ...
                                        {sitter.is_premium && <span className={style.proLabel}><IconDiamond /> ПРО Ситтер</span>}
                                    </div>
                ...
                {infoModalData && (
                    <div className={style.infoModalOverlay} onClick={() => setInfoModalData(null)}>
                        <div className={style.infoModalContent} onClick={e => e.stopPropagation()}>
                            {infoModalData.iconUrl && (
                                <div className={style.infoModalIcon}>
                                    <img src={infoModalData.iconUrl} alt="icon" />
                                </div>
                            )}
                ...
                <SelectOrderModal
                    ...
                    onCreateNew={() => {
                        setIsOrderModalOpen(false);
                        router.push(`/cabinet/orders/create?worker_id=${sitterId}`);
                    }}
                />
