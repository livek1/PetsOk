...
const unitTranslations: Record<string, string> = {
    'night': 'ночь', 'visit': 'визит', 'walk_60': 'прогулка', 'walk_30': 'прогулка', 'day': 'день', 'hour': 'час', 'unit': 'услуга'
};
...
const SitterCardWeb: React.FC<SitterCardProps> = ({ data, isHovered, onHover, onLeave }) => {
    ...
    const {
        name, title, user_rating, reviews_count, repeat_order_count,
        service_price, service_price_unit, currency_code, currency_symbol,
        is_premium, distance_km, id, slug, avatar, last_review, certificates
    } = data;

    const avatarUri = avatar?.data?.preview_url || avatar?.data?.url;
    const ratingValue = user_rating ? parseFloat(user_rating) : 0;
    const displayRating = ratingValue > 0 ? ratingValue.toFixed(1) : t('common.noRatingShort', '—');

    let reviewTextToDisplay = '';
    if (reviews_count > 0) reviewTextToDisplay = `(${reviews_count})`;
    ...
    if (service_price !== null && service_price !== undefined) {
        ...
        if (!translatedUnit && service_price_unit) translatedUnit = unitTranslations[service_price_unit] || service_price_unit;

        const unitText = translatedUnit ? ` / ${translatedUnit}` : '';
        const priceFormatted = new Intl.NumberFormat(i18n.language, { maximumFractionDigits: 0 }).format(service_price);

        priceDisplay = `${priceFormatted} ${finalCurrencySymbol}${unitText}`;
        priceLabel = t('sitterCard.price_from_label', 'от ');
    }

    const certsArray = certificates?.data || certificates || [];
    const visibleCertificates = certsArray.slice(0, 3);

    // ИСПРАВЛЕНИЕ: Используем slug если он есть, иначе id
    const profileUrl = `/sitter/${slug || id}`;

    return (
        <div
            className={`${style.cardShadowContainer} ${is_premium ? style.premium : ''} ${isHovered ? style.cardHovered : ''}`}
            onMouseEnter={onHover}
            onMouseLeave={onLeave}
        >
            <Link href={profileUrl} target="_blank" rel="noopener noreferrer" className={`${style.card} ${is_premium ? style.premiumBorder : ''}`}>
                {is_premium && (
                    <div className={style.premiumBanner}>
                        <IconCrownSmall />
                        <span className={style.premiumBannerText}>{t('common.pro', 'ПРО')}</span>
                    </div>
                )}
                ...
                <div className={style.mainContent}>
                    <div className={style.avatarWrapper}>
                        {avatarUri ? (
                            ...
                            <div className={style.avatarPlaceholder}><IconPaw /></div>
                        )}
                    </div>

                    <div className={style.descriptionContainer}>
                        <h3 className={style.name} title={name}>{name || t('sitter_name_placeholder', 'Имя не указано')}</h3>

                        {visibleCertificates.length > 0 && (
                            <div className={style.badgesContainer}>
                                {visibleCertificates.map((cert: any, index: number) => (
                                    <div key={cert.slug || index} className={style.certBadge} title={cert.name}>
                                        <span className={style.certBadgeText}>{cert.name}</span>
                                    </div>
                                ))}
                            </div>
                        )}

                        {title && <p className={style.titleText} title={title}>{title}</p>}
                        ...
                        <div className={style.ratingAndDistanceContainer}>
                            ...
                            {distance_km !== null && distance_km !== undefined && (
                                <div className={style.distanceContainer}>
                                    <IconLocation />
                                    <span className={style.distanceText}>~{Number(distance_km).toFixed(1)} {t('common.km', 'км')}</span>
                                </div>
                            )}
                        </div>
                        ...
                        {repeat_order_count > 0 && (
                            <div className={style.infoRow}>
                                <IconHeart />
                                <span className={style.infoText}>{t('sitterCard.repeat_clients', { count: repeat_order_count, defaultValue: `${repeat_order_count} повторных` })}</span>
                            </div>
                        )}
                        ...
                        {last_review && (
                            <div className={style.reviewBlock}>
                                <div className={style.reviewHeader}>
                                    <IconQuote />
                                    <span className={style.reviewLabel}>{t('sitterCard.last_review', 'Последний отзыв')}</span>
                                </div>
                                <div className={style.reviewContent}>
                                    <p className={style.reviewText}>{last_review.text}</p>
                                    <span className={style.reviewAuthor}>— {last_review.author?.name || t('common.user', 'Пользователь')}</span>
                                </div>
                            </div>
                        )}
                    </div>
