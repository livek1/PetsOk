...
export const legalContentRu = {
  terms: {
    ...
    content: [
      ...
      { type: 'p', text: "3.4. <strong>Информационные рассылки (Реклама).</strong> Пользователь соглашается получать от Сервиса информационные рассылки и рекламные материалы (в том числе от партнеров Сервиса) на указанный email и номер телефона. Согласие дается в соответствии с ФЗ «О рекламе». Пользователь вправе в любой момент отозвать данное согласие, изменив настройки уведомлений в Личном кабинете или обратившись в поддержку, при этом функционал Сервиса останется доступным в полном объеме." },
      { type: 'p', text: "3.5. <strong>Сервисные уведомления.</strong> Для корректной работы Платформы (подтверждение регистрации, статусы заказов, чеки, уведомления о безопасности) Сервис вправе направлять Пользователю автоматические системные сообщения. Настоящим Пользователь дает согласие на получение таких уведомлений посредством СМС, push-уведомлений, электронной почты, а также через мессенджеры (Telegram, WhatsApp и др.), привязанные к номеру телефона. Поскольку эти сообщения являются неотъемлемой частью процесса оказания услуг, отказ от них возможен только при полном удалении Аккаунта." },
